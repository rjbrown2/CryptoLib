...
static const char* SQL_SADB_GET_SA_BY_SPI =
        ...
        ",HEX(iv),acs_len,acs,abm_len,HEX(abm),arsn_len,HEX(arsn),arsn_win_len,HEX(arsn_win)"
...
static int32_t parse_sa_from_mysql_query(char* query, SecurityAssociation_t** security_association)
{
    ...
    while ((row = mysql_fetch_row(result)))
    {
        for (int i = 0; i < num_fields; i++)
        {
            ...
            if (strcmp(field_names[i], "arsn_win_len") == 0)
            {
                sa->arsn_win_len = atoi(row[i]);
                continue;
            }
            if (strcmp(field_names[i], "arsn_win") == 0)
            {
                sa->arsn_win = atoi(row[i]);
                continue;
            }
